...
##print translength,transwidth

##print transwidth,translength
##
##cv2.namedWindow('Image',cv2.WINDOW_NORMAL)
...
##plt.hist(graydst.flatten(),256,[0,256], color = 'r')
##plt.xlim([0,256])
##plt.legend(('cdf','histogram'), loc = 'upper left')
##plt.show()

ret,thresh5 = cv2.threshold(graydst,245,255,cv2.THRESH_TOZERO_INV)
res2 = np.hstack((graydst,thresh5))
...
##plt.hist(thresh5.flatten(),256,[0,256], color = 'r')
##plt.xlim([0,256])
##plt.legend(('cdf','histogram'), loc = 'upper left')
##plt.show()

##equ = cv2.equalizeHist(graydst)
clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
cl1 = clahe.apply(thresh5)
res = np.hstack((thresh5,cl1))
...
retdst, threshdst = cv2.threshold(cl1,145,255,cv2.THRESH_BINARY_INV)

##cv2.namedWindow('Image',cv2.WINDOW_NORMAL)
##cv2.imshow('Image',threshdst)
##cv2.waitKey(0)
##cv2.destroyAllWindows()

kerneldst = np.ones((3,3),np.uint8)
opendst = cv2.morphologyEx(threshdst, cv2.MORPH_CLOSE, kerneldst,iterations = 10)
...
cv2.namedWindow('Image',cv2.WINDOW_NORMAL)
cv2.imshow('Image',scalecutlines)
cv2.waitKey(0)
cv2.destroyAllWindows()
